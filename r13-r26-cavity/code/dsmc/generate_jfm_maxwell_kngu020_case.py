...
def write_case(
    ...
    *,
    ...
    checkpoint_block = ""
    if checkpoint_steps:
        # Restart files are the recoverable checkpoints.  Do not emit a grid
        # dump from an undefined ``f_fieldavg`` fix: the independent block
        # dumps and the final production mean already provide the field
        # evidence used by the validator and ensemble analysis.
        checkpoint_block = (
            f"restart              {checkpoint_steps} "
            f"restart.maxwell{kn_tag}.1 restart.maxwell{kn_tag}.2\n"
        )
...
"""
    ...
    metadata: dict[str, object] = {
        ...
        "checkpoint_artifact": (
            "alternating SPARTA restart files; field evidence is written by "
            "the independent block averages and the final production mean"
        ),
