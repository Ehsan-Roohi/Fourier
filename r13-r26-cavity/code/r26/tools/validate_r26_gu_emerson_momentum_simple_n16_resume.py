#!/usr/bin/env python3
"""Independent fail-closed validator for momentum-diagonal SIMPLE N16."""
...
def main() -> None:
    ...
    final = json.loads(
        (args.run_dir / "GU_EMERSON_MOMENTUM_SIMPLE_N16_RESUME_PASSED.json").read_text()
    )
    require(
        final.get("status") == "R26_GU_EMERSON_MOMENTUM_SIMPLE_N16_RESUME_PASSED",
        "momentum-SIMPLE resume status is not PASSED",
    )
    require(final.get("source_commit") == args.expected_source_commit, "source commit mismatch")
    require(final.get("rana_source_history_used") is False, "Rana source history was reused")
    require(final.get("matrix_refresh_interval") == 1, "velocity matrix was not refreshed every sweep")
    require(
        final.get("simple_momentum_coefficient")
        == "component-wise alpha_u/a_P from the velocity block solved in stage (i)",
        "SIMPLE momentum coefficient provenance mismatch",
    )
    ...
    comparison = json.loads(
        (args.run_dir / "N16_MOMENTUM_SIMPLE_ROOT_COMPARISON.json").read_text()
    )
    ...
    for name in PATHS:
        ...
        require(
            tuple(history[-1].get("stage_order", ())) == GU_EMERSON_STAGE_ORDER,
            f"{name} stage order mismatch",
        )
        ...
        require(
            abs(diagnostics.held_out_continuity) <= options.held_continuity_tolerance,
            f"{name} continuity failed",
        )
        require(abs(diagnostics.mass_error) <= options.mass_tolerance, f"{name} mass gate failed")
        require(
            diagnostics.min_density > 0.0 and diagnostics.min_temperature > 0.0,
            f"{name} positivity failed",
        )
        summaries.append(
            {
                "path_name": name,
    ...
    validation = {
        "status": "R26_GU_EMERSON_MOMENTUM_SIMPLE_N16_VALIDATION_PASS",
    ...
    output = args.run_dir / "GU_EMERSON_MOMENTUM_SIMPLE_N16_VALIDATION.json"
    output.write_text(json.dumps(jsonable(validation), indent=2, sort_keys=True) + "\n")
